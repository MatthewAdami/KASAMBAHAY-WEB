...
function StatCard({ label, value, sub, accentColor, c }) {
  return (
    <div style={{
      ...
      <p style={{ margin: '0 0 8px', fontSize: 11, fontWeight: 600, color: c.textMuted, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
        {label}
      </p>
      <p style={{ margin: '0 0 4px', fontSize: 24, fontWeight: 700, color: c.text, lineHeight: 1 }}>
        {value ?? '—'}
      </p>
...
export default function AdminDashboard() {
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', background: c.bg, fontFamily: "'DM Sans', sans-serif" }}>
      ...
      {/* ── Top bar ── */}
      <div style={{ background: c.bgTopbar, borderBottom: `1px solid ${c.border}`, padding: '10px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
      ...
      <div style={{ padding: '16px 20px', maxWidth: 1200 }}>
        ...
        <div style={{ background: c.bgCard, border: `1px solid ${c.border}`, borderRadius: 12, padding: '16px 20px', marginBottom: 12 }}>

          {/* Date + title */}
          <p style={{ margin: '0 0 2px', fontSize: 11, color: c.textMuted }}>{fmtDate()}</p>
          <h2 style={{ margin: '0 0 2px', fontSize: 13, fontWeight: 700, color: c.text, textAlign: 'center' }}>System Overview</h2>
          <p style={{ margin: '0 0 12px', fontSize: 13, color: c.textMuted, textAlign: 'center' }}>Aggregate statistics across all districts and years</p>
        ...
        {stats && (
          <div style={{ background: c.bgCard, border: `1px solid ${c.border}`, borderRadius: 12, padding: '16px 20px', marginBottom: 12 }}>
            <h3 style={{ margin: '0 0 12px', fontSize: 14, fontWeight: 600, color: c.text }}>Records by year &amp; district</h3>

            <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(years.length, 3)}, 1fr)`, gap: 20 }}>
              {years.map(yr => (
                <div key={yr}>
                  {/* Year header row */}
                  <div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: 6, borderBottom: `2px solid ${c.border}`, marginBottom: 4 }}>
                  ...
                  {stats.byYear[yr].map(row => (
                    <div key={row.district} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: `1px solid ${c.border}` }}>
        ...
        {stats && (
          <div style={{ background: c.bgCard, border: `1px solid ${c.border}`, borderRadius: 12, padding: '16px 20px', marginBottom: 12 }}>
            <h3 style={{ margin: '0 0 12px', fontSize: 14, fontWeight: 600, color: c.text }}>Classification breakdown</h3>
            ...
            {classification.map(({ label, count }) => (
              <div key={label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: `1px solid ${c.border}` }}>
                ...
                <span style={{ fontSize: 13, fontWeight: 700, color: c.text }}>{count.toLocaleString()}</span>
              </div>
            ))}
          </div>
        )}
